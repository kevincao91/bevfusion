...
def make_data(file_list, out_name):
    infos = []
    ...
    if end_type == '.json':
        for ids, file_name in enumerate(file_list):
            ...
            for ann in anns:
                ...
                obj_position = [obj_position['x'], obj_position['y'], obj_position['z']]
                obj_scale = ann['psr']['scale']
                obj_scale = [obj_scale['x'], obj_scale['y'], obj_scale['z']]
                obj_rotation = ann['psr']['rotation']
                obj_rotation = [obj_rotation['x'], obj_rotation['y'], -obj_rotation['z']]
                obj_boxe = obj_position + obj_scale + obj_rotation[-1:]
        ...
        for ids, file_name in enumerate(file_list):
            ...
            for ann in anns:
                ...
                obj_scale = [obj_scale['x'], obj_scale['y'], obj_scale['z']]
                obj_rotation = ann['psr']['rotation']
                obj_rotation = -obj_rotation['z']  # 标注出来的是弧度
                gt_positions.append(obj_position)
                gt_scales.append(obj_scale)
                gt_rotations.append(obj_rotation)
                gt_names.append(obj_type)
                gt_velocity.append([66, 66])
                num_lidar_pts.append(6)
                num_radar_pts.append(6)
            ...
            # 添加gt数据
            locs = np.array(gt_positions).reshape(-1, 3)
            dims = np.array(gt_scales).reshape(-1, 3)
            rots = np.array(gt_rotations).reshape(-1, 1)
...
if __name__ == "__main__":
    ...
    # make_data(train_files, 'nuscenes_infos_train.pkl')
    # make_data(valid_files, 'nuscenes_infos_val.pkl')
