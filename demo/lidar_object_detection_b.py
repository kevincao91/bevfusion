...
class LidarEncoder():
    def __init__(self, encoders):
        ...
        voxelize_cfg = encoders.lidar.voxelize
        self.sparse_shape = encoders.lidar.backbone.sparse_shape

        self.gen = PointToVoxel(vsize_xyz=voxelize_cfg.voxel_size,   # [0.075, 0.075, 0.2],
                                coors_range_xyz=voxelize_cfg.point_cloud_range,   #  [-54.0, -54.0, -5.0, 54.0, 54.0, 3.0],
                                num_point_features=5,
                                max_num_voxels=voxelize_cfg.max_voxels[-1],     # 160000,
                                max_num_points_per_voxel=voxelize_cfg.max_num_points)  #  10
...
def main() -> None:
    ...
    # ===
    obj_head_cfg = cfg.model.heads.object
    num_proposals = obj_head_cfg.num_proposals
    num_classes = obj_head_cfg.num_classes
    loss_cls = obj_head_cfg.loss_cls
    bbox_coder = obj_head_cfg.bbox_coder
    ...
    for points, metas in tqdm(dataflow):
        ...
        # tin = time.time()
        # print('extract_lidar_features in')
        lidar_features = lie.extract_lidar_features_v2(points_cpu)
